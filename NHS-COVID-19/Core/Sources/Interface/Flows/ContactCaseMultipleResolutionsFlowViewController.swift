    ...
    private func rootViewController(for state: State) -> UIViewController {
        ...
        case .vaccinationStatus:
            let interactor = VaccinationStatusViewControllerInteractor(
                ...
                didTapConfirm: { [weak self] isFullyVaccinated in
                    self?.interactor.didDeclareVaccinationStatus(isFullyVaccinated: isFullyVaccinated)
                }
            )
...
private struct VaccinationStatusViewControllerInteractor: ContactCaseVaccinationStatusViewController.Interacting {
    ...
    private let _didTapConfirm: (_ isFullyVaccinated: Bool) -> Void
    
    init(
        didTapAboutApprovedVaccinesLink: @escaping () -> Void,
        didTapConfirm: @escaping (_ isFullyVaccinated: Bool) -> Void
    ) {
    ...
    func didTapConfirm(isFullyVaccinated: Bool) {
        _didTapConfirm(isFullyVaccinated)
    }
