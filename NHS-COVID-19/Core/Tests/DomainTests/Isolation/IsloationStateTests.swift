...
class IsolationStateTests: XCTestCase {
    ...
    func testInitializingNotIsolatingWhenNotAcknowledged() {
        let state = IsolationState(logicalState: .isolationFinishedButNotAcknowledged(Isolation(fromDay: .today, untilStartOfDay: .today, reason: Isolation.Reason(indexCaseInfo: nil, contactCaseInfo: .init(optOutOfIsolationDay: nil)))))
