...
class _IsolationLogicalStateTests: XCTestCase {
    ...
    func testEmptyIndexCaseInfo() {
        ...
        XCTAssertEqual(state.isolation, Isolation(fromDay: .today, untilStartOfDay: LocalDay(gregorianDay: endDay, timeZone: $state.today.timeZone), reason: .indexCase(hasPositiveTestResult: false, testkitType: nil, isSelfDiagnosed: true)))
    }
    ...
    func testIndexCaseNegativeTestResultNoOnsetDay() {
        ...
        XCTAssertEqual(state.isolation, Isolation(fromDay: .today, untilStartOfDay: LocalDay(gregorianDay: $state.today.gregorianDay, timeZone: $state.today.timeZone), reason: .indexCase(hasPositiveTestResult: false, testkitType: .labResult, isSelfDiagnosed: true)))
    }
    ...
    func testIndexCasePositiveTestResultNoOnsetDay() {
        ...
        let endDay = $state.selfDiagnosisDay.advanced(by: 9)
        XCTAssertEqual(state.isolation, Isolation(fromDay: .today, untilStartOfDay: LocalDay(gregorianDay: endDay, timeZone: $state.today.timeZone), reason: .indexCase(hasPositiveTestResult: true, testkitType: .labResult, isSelfDiagnosed: true)))
    }
    ...
    func testContactCaseWithIndexCaseWithOnsetDay() {
        ...
        $state.isolationInfo.indexCaseInfo = IndexCaseInfo(selfDiagnosisDay: $state.selfDiagnosisDay, onsetDay: onsetDay, testResult: nil)
        let endDay = $state.selfDiagnosisDay.advanced(by: 12)
        XCTAssertEqual(state.isolation, Isolation(fromDay: .today, untilStartOfDay: LocalDay(gregorianDay: endDay, timeZone: $state.today.timeZone), reason: .bothCases(hasPositiveTestResult: false, testkitType: nil, isSelfDiagnosed: true)))
    }
    ...
    func testContactCaseWithIndexCaseNoOnsetDayPositiveTestResult() {
        ...
        $state.isolationInfo.indexCaseInfo = IndexCaseInfo(selfDiagnosisDay: $state.selfDiagnosisDay, onsetDay: nil, testResult: .positive)
        let endDay = $state.selfDiagnosisDay.advanced(by: 12)
        XCTAssertEqual(state.isolation, Isolation(fromDay: .today, untilStartOfDay: LocalDay(gregorianDay: endDay, timeZone: $state.today.timeZone), reason: .bothCases(hasPositiveTestResult: true, testkitType: .labResult, isSelfDiagnosed: true)))
    }
    ...
    func testEndOfIsolationBeforeTodayDoesNotTriggerIsolation() {
        ...
        XCTAssertEqual(state, .isolationFinishedButNotAcknowledged(Isolation(fromDay: .today, untilStartOfDay: endDay, reason: .indexCase(hasPositiveTestResult: true, testkitType: .labResult, isSelfDiagnosed: true))))
    }
    ...
    func testEndOfIsolationAtStartOfTodayDoesNotTriggerIsolation() {
        ...
        XCTAssertEqual(state, .isolationFinishedButNotAcknowledged(Isolation(fromDay: .today, untilStartOfDay: $state.today, reason: .indexCase(hasPositiveTestResult: true, testkitType: .labResult, isSelfDiagnosed: true))))
    }
    ...
    func testEndOfIsolationBeforeTodayDoesNotTriggerIsolationWhenAcknowledged() {
        ...
        let endDay = $state.today.advanced(by: -2)
        XCTAssertEqual(state, .notIsolating(finishedIsolationThatWeHaveNotDeletedYet: Isolation(fromDay: .today, untilStartOfDay: endDay, reason: .indexCase(hasPositiveTestResult: true, testkitType: .labResult, isSelfDiagnosed: true))))
    }
    ...
    func testManualTestEntryTriggersIsolation() {
        ...
        $state.isolationInfo.indexCaseInfo = IndexCaseInfo(
            ...
            onsetDay: nil,
            testInfo: .init(result: .positive, testKitType: .labResult, receivedOnDay: .today)
        )
        
        let endDay = npexDay.advanced(by: $state.isolationConfiguration.indexCaseSinceNPEXDayNoSelfDiagnosis.days).gregorianDay
        XCTAssertEqual(state.isolation, Isolation(fromDay: npexDay, untilStartOfDay: LocalDay(gregorianDay: endDay, timeZone: $state.today.timeZone), reason: .indexCase(hasPositiveTestResult: true, testkitType: .labResult, isSelfDiagnosed: false)))
    }
    ...
    func testManualTestEntryWithVoidResultDoesNotTriggerIsolation() {
        ...
        $state.isolationInfo.indexCaseInfo = IndexCaseInfo(
            isolationTrigger: .manualTestEntry(npexDay: npexDay.gregorianDay),
            onsetDay: nil,
            testInfo: .init(result: .void, testKitType: .labResult, receivedOnDay: .today)
        )
    ...
    func testManualTestEntryWithOldTestDoesNotTriggerIsolation() {
        ...
        let endDay = npexDay.advanced(by: $state.isolationConfiguration.indexCaseSinceNPEXDayNoSelfDiagnosis.days).gregorianDay
        XCTAssertEqual(state, .isolationFinishedButNotAcknowledged(Isolation(fromDay: npexDay, untilStartOfDay: LocalDay(gregorianDay: endDay, timeZone: $state.today.timeZone), reason: .indexCase(hasPositiveTestResult: true, testkitType: .labResult, isSelfDiagnosed: false))))
    }
    ...
    func testInterestedInExposureNotificationsFalseWhenIsolatingFromBothCases() {
        let state: IsolationLogicalState = isolating(for: .bothCases(hasPositiveTestResult: true, testkitType: .labResult, isSelfDiagnosed: true))
        
        XCTAssertFalse(state.interestedInExposureNotifications)
    }
    
    func testInterestedInExposureNotificationsFalseWhenIsolatingFromIndexCaseWithPositiveTest() {
        let state: IsolationLogicalState = isolating(for: .indexCase(hasPositiveTestResult: true, testkitType: .labResult, isSelfDiagnosed: true))
        
        XCTAssertFalse(state.interestedInExposureNotifications)
    }
    
    func testInterestedInExposureNotificationsTrueWhenIsolatingFromIndexCaseWithNoPositiveTest() {
        let state: IsolationLogicalState = isolating(for: .indexCase(hasPositiveTestResult: false, testkitType: .labResult, isSelfDiagnosed: true))
...
private extension IndexCaseInfo {
    ...
    init(selfDiagnosisDay: GregorianDay, onsetDay: GregorianDay?, testResult: TestResult?) {
        self.init(
            ...
            onsetDay: onsetDay,
            testInfo: testResult.map { TestInfo(result: $0, testKitType: .labResult, receivedOnDay: .today) }
        )
    }
